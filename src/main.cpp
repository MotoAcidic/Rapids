...
void ReprocessBlocks(int nBlocks)
{
    ...
    while (it != mapRejectedBlocks.end()) {
        ...
        if ((*it).second > GetTime() - (nBlocks * sporkManager.GetSporkValue(SPORK_22_TARGET_SPACING) * 2)) {
...
{
    {
        ...
        if (state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * sporkManager.GetSporkValue(SPORK_22_TARGET_SPACING) * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
